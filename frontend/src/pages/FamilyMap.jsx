...
export default function FamilyMap() {
  ...
  // Auto-refresh — pauses when tab is hidden to save battery
  useEffect(() => {
    if (!isParent) return
    const startTimer = () => {
  ...
  useEffect(() => {
    if (isLoaded && located.length > 0) fitBounds()
  }, [isLoaded, fitBounds])
      ...
      <main className="page-scroll px-4 max-w-lg mx-auto space-y-4 pb-24">
        ...
        {children.length > 0 && (
          <>
            ...
            {/* Fit all button — only when multiple children */}
            {located.length > 1 && (
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={() => setSelected(null)}>
            ...
            <div className="relative bg-white dark:bg-gray-800 rounded-t-3xl w-full max-w-lg p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-3 mb-4 mt-2">
                <div className="w-12 h-12 rounded-full overflow-hidden shrink-0 ring-2 ring-white dark:ring-gray-700 shadow"
              ...
              {selected.accuracy_m != null && (
                <p className="text-gray-400 dark:text-gray-500 text-xs mb-3">
                  דיוק מיקום: ±{Math.round(selected.accuracy_m)} מטר
                </p>
              )}
